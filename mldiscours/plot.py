...
class Scope2(object):
    def __init__(self, ax, verbose=1, interval=5, save=False,
                 filename='myplot.png'):
        ...
        self.line_b_loss = Line2D(self.b_x_loss, self.b_y_loss, linewidth=.1,
                                  linestyle=':', color='b')
        self.line_e_loss = Line2D(self.e_x_loss, self.e_y_loss, linewidth=.5,
                                  color='b')
        self.line_e_val_loss = Line2D(self.e_x_val_loss, self.e_y_val_loss,
                                      linewidth=.3, color='c')
        # define the accuracy lines
        self.line_b_acc = Line2D(self.b_x_acc, self.b_y_acc, linewidth=.1,
                                 linestyle=':', color='r')
        self.line_e_acc = Line2D(self.e_x_acc, self.e_y_acc, linewidth=.6,
                                 color='r')
        self.line_e_val_acc = Line2D(self.e_x_val_acc, self.e_y_val_acc,
                                     linewidth=.3, color='m')
    ...
    def _update(self, b_x_loss, b_y_loss,
                e_x_loss, e_y_loss,
                b_x_acc, b_y_acc,
                e_x_acc, e_y_acc,
                e_x_val_acc, e_y_val_acc,
                e_x_val_loss, e_y_val_loss,
                force=False):
        ...
        if self.verbose == 1:
            if not force and (now - self.last_update) < self.interval:
                ...
                self.line_e_val_loss.set_data(self.e_x_val_loss,
                                              self.e_y_val_loss)
...
class DummyCallback(Callback):
    def on_train_begin(self, logs={}):
        ...
        self.i = 0
        self.b_x_loss = np.zeros(
            (int((self.nb_sample / self.batch_size)) + 1) * self.nb_epoch)
        self.b_y_loss = np.zeros(
            (int((self.nb_sample / self.batch_size)) + 1) * self.nb_epoch)
...
class PlotLive(Callback):
    ...
    def on_train_begin(self, logs={}):
        ...
        self.i = 0
        self.e = 0
        # numpy arrays that hold plot parameters, same as in Scope2
        self.b_x_loss = np.zeros(
            (int((self.nb_sample / self.batch_size)) + 1) * self.nb_epoch)
        self.b_y_loss = np.zeros(
            (int((self.nb_sample / self.batch_size)) + 1) * self.nb_epoch)
        ...
        self.b_x_acc = np.zeros(
            (int((self.nb_sample / self.batch_size)) + 1) * self.nb_epoch)
        self.b_y_acc = np.zeros(
            (int((self.nb_sample / self.batch_size)) + 1) * self.nb_epoch)
        ...
        # add prediction plot after each epoch?
...
def plotme():
    ...
    scope = Scope2(ax, interval=5, save=True)
    ani = FuncAnimation(fig, scope._update)
    # ion needed in scope
    plt.show()
    history = PlotLive(scope=scope)
    # dummy = DummyCallback()
    res = model.fit(X_train, Y_train, nb_epoch=4, verbose=1, batch_size=100,
                    callbacks=[history])
